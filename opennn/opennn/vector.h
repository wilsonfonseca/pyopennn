...
public:
  ...
  Vector<T> get_binary_vector(const Vector<T>&) const;
...
template <class T>
Vector<T> Vector<T>::get_binary_vector(const Vector<T>& unique_items) const
{
...
template <class T>
Matrix<T> Vector<T>::get_unique_binary_matrix(const char& separator, const Vector<T>& columns_elements) const
{
    const size_t this_size = this->size();
...
Vector< Vector<T> > Vector<T>::split(const size_t& n) const
{
    ...
    for (int k = 0; k < batchs_number; ++k)
    {
        ...
        {
            end_itr = this->cend();
            batchs[k].resize(this->size() - k*n);
        }

        // copy elements from the input range to the sub-vector
        std::copy(start_itr, end_itr, batchs[k].begin());
    }
...
template<class T>
Vector<T> hyperbolic_tangent_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> linear_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> hyperbolic_tangent_second_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> logistic_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> logistic_second_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> threshold_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
     ...
     for(size_t i = 0; i < n; i++)
     {
         ...
         else
         {
             ...
             buffer << "OpenNN Exception: Matrix Template.\n"
                    << "Matrix<T> threshold_derivative(const Matrix<T>&).\n"
...
template<class T>
Vector<T> threshold_second_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> symmetric_threshold_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> symmetric_threshold_second_derivative(const Vector<T>& x)
{
    const size_t n = x.size();
...
template<class T>
Vector<T> linear_second_derivative(const Vector<T>& x)
{
