...
#pragma omp parallel for
    ...
    for(int i = 0; i < static_cast<int>(input_vectors_number); i++)
    {
...
#pragma omp parallel for private(inputs, outputs)
    ...
    for(int i = 0; i < static_cast<int>(input_vectors_number); i++)
    {
        inputs = input_data.get_row(static_cast<size_t>(i));
        outputs = calculate_outputs(inputs, time[static_cast<size_t>(i)]);
        output_data.set_row(static_cast<size_t>(i), outputs);
    }
...
Matrix<double> NeuralNetwork::calculate_Jacobian(const Vector<double>& inputs) const
{
    ...
    if(scaling_layer_pointer)
    {
        ...
        scaling_layer_Jacobian = scaling_layer_pointer->get_Jacobian(scaling_layer_derivative);
    ...
    if(unscaling_layer_pointer)
    {
        ...
        unscaling_layer_Jacobian = unscaling_layer_pointer->get_Jacobian(unscaling_layer_derivative);
    ...
    if(bounding_layer_pointer)
    {
        ...
        bounding_layer_Jacobian = bounding_layer_pointer->get_Jacobian(derivatives);
...
Matrix<double> NeuralNetwork::calculate_Jacobian(const Vector<double>& inputs, const double& time) const
{
    ...
    if(inputs_trending_layer_pointer)
    {
        ...
        inputs_trending_layer_Jacobian = inputs_trending_layer_pointer->get_Jacobian(derivatives);
    ...
    if(outputs_trending_layer_pointer)
    {
        ...
        outputs_trending_layer_Jacobian = outputs_trending_layer_pointer->get_Jacobian(derivatives);
...
Vector< Matrix<double> > NeuralNetwork::calculate_Hessian(const Vector<double>& inputs) const
{
    return(multilayer_perceptron_pointer->calculate_Hessian(inputs));
}
...
Vector< Matrix<double> > NeuralNetwork::calculate_Hessian(const Vector<double>& inputs, const Vector<double>& parameters) const
{
    return(multilayer_perceptron_pointer->calculate_Hessian(inputs, parameters));
}
