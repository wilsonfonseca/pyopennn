...
const size_t& QuasiNewtonMethod::get_maximum_selection_loss_decreases() const
{
...
/// @param new_maximum_selection_loss_decreases Maximum number of epochs in which the selection evalutation decreases.

void QuasiNewtonMethod::set_maximum_selection_error_increases(const size_t& new_maximum_selection_loss_decreases)
{
...
QuasiNewtonMethod::QuasiNewtonMethodResults* QuasiNewtonMethod:: perform_training()
{
   ...
   for(size_t epoch = 0; epoch < maximum_epochs_number; epoch++)
   {
           ...
           //for(size_t i = 0; i < 10000; i++) loss_index_pointer->calculate_training_loss_gradient();
           ...
           else if(epoch != 0 && selection_loss > old_selection_loss)
           {
               selection_failures++;
           }
           ...
           {
               results_pointer->selection_error_history[epoch] = selection_loss;
           }
...
//            if(display)
//            {
//               cout << "Epoch " << epoch << ": Minimum loss decrease reached.\n"
...
void QuasiNewtonMethod::from_XML(const tinyxml2::XMLDocument& document)
{
   ...
   // Maximum selection loss decreases
   {
       ...
       if(element)
       {
          const size_t new_maximum_selection_loss_decreases = static_cast<size_t>(atoi(element->GetText()));
