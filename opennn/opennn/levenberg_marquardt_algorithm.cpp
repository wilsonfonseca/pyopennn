...
void LevenbergMarquardtAlgorithm::set_maximum_selection_error_increases(const size_t& new_maximum_selection_loss_decreases)
{
...
::calculate_gradient(const Vector<double>& terms, const Matrix<double>& terms_Jacobian) const
{
   ...
   const Vector<double> error_gradient = terms_Jacobian.calculate_transpose().dot(terms)*2.0;

   const Vector<double> regularization_gradient = loss_index_pointer->calculate_regularization_gradient();
...
Matrix<double> LevenbergMarquardtAlgorithm::calculate_Hessian_approximation(const Matrix<double>& terms_Jacobian) const
{
   ...
   const Matrix<double> error_Hessian = (terms_Jacobian.calculate_transpose().dot(terms_Jacobian)*2.0).sum_diagonal(damping_parameter);

   const Matrix<double> regularization_Hessian = loss_index_pointer->calculate_regularization_Hessian();

   return(error_Hessian + regularization_Hessian);
}
...
LevenbergMarquardtAlgorithm::LevenbergMarquardtAlgorithmResults* LevenbergMarquardtAlgorithm::perform_training()
{
   ...
   {
       const Vector<size_t> training_batch_indices = training_indices.get_subvector_random(training_batch_size);
      ...
      terms = loss_index_pointer->calculate_error_terms(training_batch_indices);

      training_loss = calculate_loss(terms);

      terms_Jacobian = loss_index_pointer->calculate_error_terms_Jacobian(training_batch_indices);
      ...
      do
      {
         Hessian_approximation = calculate_Hessian_approximation(terms_Jacobian);

         parameters_increment = perform_Householder_QR_decomposition(Hessian_approximation, gradient*(-1.0));

         const double new_loss = loss_index_pointer->calculate_loss(training_batch_indices, parameters+parameters_increment);
      ...
      selection_loss = loss_index_pointer->calculate_loss(selection_batch_indices);
...
//      else if(iteration != 0 && training_loss_decrease <= minimum_loss_decrease)
//      {
//         if(display)
//         {
...
//      }
	  ...
	  if(stop_training)
      {
          if(display)
          {
             ...
             if(fabs(selection_loss - 0) < numeric_limits<double>::epsilon())
             {
...
void LevenbergMarquardtAlgorithm::from_XML(const tinyxml2::XMLDocument& document)
{
   ...
   {
      const size_t new_maximum_selection_loss_decreases = static_cast<size_t>(atoi(maximum_selection_loss_decreases_element->GetText()));
