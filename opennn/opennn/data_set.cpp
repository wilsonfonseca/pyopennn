...
{
    const Matrix<double> target_data = get_target_data();
...
Matrix<double> DataSet::get_input_data(const Vector<size_t>& instances_indices) const
{
    const Vector<size_t> input_indices = variables.get_inputs_indices();
...
Matrix<double> DataSet::get_target_data(const Vector<size_t>& instances_indices) const
{
...
Matrix<double> DataSet::get_used_input_data() const
{
...
Matrix<double> DataSet::get_used_target_data() const
{
   const Vector<size_t> indices = instances.get_used_indices();
...
Matrix<double> DataSet::get_training_input_data() const
{
...
Matrix<double> DataSet::get_training_target_data() const
{
   const Vector<size_t> training_indices = instances.get_training_indices();
...
Matrix<double> DataSet::get_testing_input_data() const
{
   const Vector<size_t> inputs_indices = variables.get_inputs_indices();
...
Matrix<double> DataSet::get_testing_target_data() const
{
   const Vector<size_t> targets_indices = variables.get_targets_indices();
...
#pragma omp parallel for schedule(dynamic)
    ...
    for(int i = 0; i < static_cast<int>(inputs_number); i++)
    {
        ...
        for(size_t j = static_cast<size_t>(i); j < inputs_number; j++)
        {
            ...
            covariance_matrix(static_cast<size_t>(i),j) = first_input_data.calculate_covariance(second_input_data);
...
void DataSet::transform_principal_components_data(const Matrix<double>& principal_components)
{
    ...
    const Matrix<double> input_data = get_input_data();
...
//    new_data = new_data.assemble_columns(get_target_data());
}
