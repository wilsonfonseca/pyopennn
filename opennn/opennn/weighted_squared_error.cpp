...
double WeightedSquaredError::calculate_error(const Matrix<double>& outputs, const Matrix<double>& targets) const
{
    // Control sentence
    ...
    return(outputs.calculate_weighted_sum_squared_error(targets, positives_weight, negatives_weight)/normalization_coefficient);
...
Matrix<double> WeightedSquaredError::calculate_output_Hessian(const Vector<size_t>&, const Vector<double>&, const Vector<double>& target) const
{
...
LossIndex::SecondOrderLoss WeightedSquaredError::calculate_second_order_loss() const
{
    // Control sentence
...
// Vector<double> calculate_error_terms(const Vector<double>&) const method
...
Vector<double> WeightedSquaredError::calculate_error_terms(const Vector<double>& network_parameters) const
{
    // Control sentence(if debug)
    ...
    return(weighted_squared_error_copy.calculate_error_terms());
}
...
Matrix<double> WeightedSquaredError::calculate_error_terms_Jacobian() const
{
    ...
    const double negatives = training_instances_number
                           - data_set_pointer->get_training_target_data().get_column(0).calculate_sum();
