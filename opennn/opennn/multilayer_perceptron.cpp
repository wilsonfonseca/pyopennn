...
Matrix<size_t> MultilayerPerceptron::get_parameters_indices() const
{
    ...
    for(size_t i = 0; i < layers_number; i++)
    {
        ...
        {
//            perceptron_parameters_number = layers[i].get_perceptron(j).get_parameters_number();
//            perceptron_parameters_number = layers[i].get_perceptrons_parameters()[j].size();
...
Vector<double> MultilayerPerceptron::calculate_outputs(const Vector<double>& inputs) const
{
    ...
    Vector<double> outputs = layers[0].calculate_outputs(inputs);
...
Vector< Matrix<double> > MultilayerPerceptron::calculate_Hessian(const Vector<double>& inputs) const
{   
#ifdef __OPENNN_DEBUG__
    ...
    const Vector< Vector< Matrix<double> > > layers_Hessian = calculate_layers_Hessian(inputs);

    // Calculate multilayer_perceptron_pointer Hessian form

    Vector< Matrix<double> > Hessian_form = layers_Hessian[layers_number-1];
    Matrix<double> Jacobian = layers_Jacobian[layers_number-1];
    ...
    for(size_t layer_index = layers_number-2; layer_index != 0; layer_index--)
    {
        for(size_t output_index = 0; output_index < outputs_number; output_index++)
        {
            ...
            {
                Hessian_form[output_index] += layers_Hessian[layer_index][neuron_index]*Jacobian(output_index,neuron_index);
            }
        }
...
Matrix<double> MultilayerPerceptron::calculate_layer_combination_combination_Jacobian(const size_t& layer_index, const Vector<double>& previous_layer_activation_derivative) const
{
    ...
    const Matrix<double> layer_synaptic_weights = layers[layer_index].get_synaptic_weights();
...
Vector<double> MultilayerPerceptron::calculate_outputs_layer_combinations(const size_t& layer_index, const Vector<double>& layer_combinations) const
{
    // Control sentence(if debug)
...
Matrix<double> MultilayerPerceptron::calculate_outputs_layer_combinations(const size_t& layer_index, const Matrix<double>& layer_combinations) const
{
    // Control sentence(if debug)
    ...
    Matrix<double> outputs = layers[layer_index].calculate_activations(layer_combinations);
...
Vector< Matrix<double> > MultilayerPerceptron::calculate_layers_delta(const Vector< Vector<double> >& layers_activation_derivative) const
{
    // Control sentence(if debug)
    ...
    if(layers_activation_derivative_size != layers_number)
    {
        ...
        buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
               << "Vector< Vector<double> > calculate_layers_delta(const Vector< Vector<double> >&) method.\n"
    ...
    if(layers_number > 0)
    {
        ...
        output_layers_delta[layers_number-1] = layers[layers_number-1].get_activations_Jacobian(layers_activation_derivative[layers_number-1]);

        // Rest of layers

        for(size_t i = layers_number-2; i == 0; i--)
        {
            output_layers_delta[i] = output_layers_delta[i+1].dot(layers[i+1].get_synaptic_weights()).dot(layers[i].get_activations_Jacobian(layers_activation_derivative[i]));
        }
    }
 ...
 const Vector< Matrix<double> >& output_layers_delta) const
{
    ...
    const Vector< Matrix<double> > layers_synaptic_weights = get_layers_synaptic_weights();
    ...
    if(layers_number > 0)
    {
        ...
        output_interlayers_Delta(layers_number-1,layers_number-1) = layers[layers_number-1].get_activations_Hessian(layers_activation_second_derivative[layers_number-1]);
...
Vector<double> MultilayerPerceptron::calculate_outputs(const Vector<double>& inputs, const Vector<double>& parameters) const
{
    ...
    Vector<double> outputs = layers[0].calculate_outputs(inputs, layers_parameters.biases[0], layers_parameters.synaptic_weights[0]);
...
{   
    ...
    const Vector< Vector<double> > layers_inputs = get_layers_input(inputs, layers_activation);
    ...
    const Vector< Matrix<double> > output_layers_delta = calculate_layers_delta(layers_activation_derivative);
...
Vector< Matrix<double> > MultilayerPerceptron::calculate_Hessian(const Vector<double>& inputs, const Vector<double>&) const
{
    // Neural network stuff
    ...
    Vector< Matrix<double> > parameters_Hessian(outputs_number);

    for(size_t i = 0; i < outputs_number; i++)
    {
        parameters_Hessian[i].set(parameters_number, parameters_number);
    }
    ...
    if(layers_number > 0)
    {
        const Matrix<size_t> parameters_indices = get_parameters_indices();
        ...
        for(size_t i = 0; i < outputs_number; i++)
        {
            for(size_t j = 0; j < parameters_number; j++)
            {
                ...
                for(size_t k = j; k < parameters_number; k++)
                {
                    ...
                    parameters_Hessian[i](k,j) = parameters_Hessian[i](j,k);
                }
            }
        }
    }
...
Vector< Vector<double> > MultilayerPerceptron::get_layers_input(const Vector<double>& inputs, const Vector< Vector<double> >& layers_activation) const
{
    // Neural network stuff
    ...
    if(inputs_size != inputs_number)
    {
        ...
        buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
               << "Vector< Vector<double> > get_layers_input(const Vector<double>&, const Vector< Vector<double> >&) const method.\n"
               << "Size must be equal to number of inputs.\n";
...
Vector< Vector<double> > MultilayerPerceptron::get_layers_input(const Vector<double>& input, const Vector< Vector< Vector<double> > >& first_order_forward_propagation) const
{
    const size_t layers_number = get_layers_number();
...
Vector< Vector<double> > MultilayerPerceptron::calculate_layers_combinations(const Vector<double>& inputs) const
{
    // Control sentence(if debug)
...
Vector< Matrix<double> > MultilayerPerceptron::calculate_layers_combinations_Jacobian(const Vector<double>& inputs) const
{
    const size_t layers_number = get_layers_number();
...
Vector< Matrix<double> > MultilayerPerceptron::calculate_layers_combination_parameters_Jacobian(const Vector< Vector<double> >& layers_inputs) const
{
    ...
    for(size_t i = 0; i < layers_number; i++)
    {
        layers_combination_parameters_Jacobian[i] = layers[i].calculate_combinations_parameters_Jacobian(layers_inputs[i]);
    }
...
//Vector < Vector< Matrix<double> > > MultilayerPerceptron::calculate_layers_combination_parameters_Jacobian(const Vector< Matrix<double> >& layers_inputs) const
//{
//    const size_t layers_number = get_layers_number();

//    Vector < Vector< Matrix<double> > > layers_combination_parameters_Jacobian(instances_number);

//    for(size_t i = 0; i < instances_numbeer; i++)
//    {
...
Vector< Vector<double> > MultilayerPerceptron::calculate_layers_activations(const Vector<double>& inputs) const
{
    // Control sentence(if debug)
...
Vector< Vector<double> > MultilayerPerceptron::calculate_layers_activations_derivatives(const Vector<double>& inputs) const
{
    // Control sentence(if debug)
...
Vector< Matrix<double> > MultilayerPerceptron::calculate_layers_activations_derivatives(const Matrix<double>& inputs) const
{
    ...
    if(inputs_size != inputs_number)
    {
        ...
        buffer << "OpenNN Exception: MultilayerPerceptron class.\n"
               << "Vector< Matrix<double> > calculate_layers_activation_derivative(const Vector<double>&) const method.\n"
...
Vector< Vector< Matrix<double> > > MultilayerPerceptron::calculate_layers_Hessian(const Vector<double>& inputs) const
{
    const size_t layers_number = get_layers_number();

    Vector<Vector<double> > layers_output(layers_number);
    Vector< Vector< Matrix<double> > > layers_Hessian(layers_number);

    layers_output[0] = layers[0].calculate_outputs(inputs);
    layers_Hessian[0] = layers[0].calculate_Hessian(inputs);

    for(size_t i = 1; i < layers_number; i++)
    {
        layers_output[i] = layers[i].calculate_outputs(layers_output[i-1]);

        layers_Hessian[i] = layers[i].calculate_Hessian(layers_output[i-1]);
    }

    return(layers_Hessian);
}
...
Matrix< Matrix<double> > MultilayerPerceptron::calculate_interlayers_combination_combination_Jacobian(const Vector<double>& inputs) const  
{
    ...
    const Vector< Vector<double> > layers_combination = calculate_layers_combinations(inputs);
